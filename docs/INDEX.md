...
| **P3.2 — Driver IA `aiStep` + test deux sièges** | 2026-06-13 | — | — | ✅ Livré | `apps/web/src/solo/ai-driver.ts` : `aiStep(state, moves, params)` framework-agnostic (≤1 coup/appel). Identifie le siège local via le discriminant « `hand` est un Array » (vs OpponentView `{count}`) plutôt que `ctx.currentPlayer` (peu fiable en phase pick simultanée). Test d'intégration headless IA-vs-IA jusqu'au vainqueur. A débusqué le bug moteur auto-stand onBegin (fix `90c23a8`, cf. QUIRKS). |
